...
pub trait IStarknetAuction<TContractState> {
    fn start(ref self: TContractState, bidding_duration: u64, starting_bid: u256);
    fn bid(ref self: TContractState, amount: u256);
    fn withdraw(ref self: TContractState);
    fn end(ref self: TContractState);
    fn get_bid(self: @TContractState) -> u256;
}
...
pub mod StarknetAuction {
    ...
    struct Storage {
        ...
        ended: bool,
        started: bool,
        bid_values: Map<ContractAddress, u256>,
    }
    ...
    impl AuctionImpl of super::IStarknetAuction<ContractState> {
        fn start(ref self: ContractState, bidding_duration: u64, starting_bid: u256) {
            let caller = get_caller_address();
        ...
        fn bid(ref self: ContractState, amount: u256) {
            ...
            assert(time < self.bidding_end.read(), 'Aunction ended');
            ...
            if previous_amount == 0 {
                amount_to_transfer = amount.into();
            } else {
                amount_to_transfer = amount.into() - previous_amount;
            }

            erc20_dispatcher.transfer_from(sender,receiver, amount_to_transfer.into());
        }
        ...
        fn end(ref self: ContractState) {
            ...
            assert(!self.ended.read(), 'Aunction is already ended');
            ...
            self
                .emit(
                    End {
                        highest_bid: self.highest_bid.read(), highest_bidder: self.highest_bidder.read()
                    }
                );
    
            erc721_dispatcher
                .transfer_from(sender, self.highest_bidder.read(), self.nft_id.read().into());
        }
    
        fn get_bid(self: @ContractState) -> u256 {
